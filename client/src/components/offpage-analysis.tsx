...
import { CheckCircle, XCircle, ExternalLink, Globe, Link, Users, TrendingUp } from "lucide-react";
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">{domainAuthority.pageRank}/10</span>
                  </div>
                  <div className="flex justify-between">
                    <span>{t('offPageAnalysis.trustRank')}</span>
                    <span className="font-semibold">{domainAuthority.trustRank}/10</span>
                  </div>
                </div>
              </div>
            </CardContent>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm">{t('offPageAnalysis.domainAge')}</span>
                  <span className="text-sm font-semibold">
                    {trustMetrics.domainAge} {t('offPageAnalysis.years')}
                  </span>
                </div>
              </div>
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-sm">{t('offPageAnalysis.spamScore')}</span>
                  <Badge variant={trustMetrics.spamScore < 30 ? "default" : trustMetrics.spamScore < 60 ? "secondary" : "destructive"}>
                    {trustMetrics.spamScore}/100
                  </Badge>
                </div>
                {trustMetrics.trustSignals.length > 0 && (
                  <div>
                    <div className="text-sm font-medium mb-2">{t('offPageAnalysis.trustSignals')}</div>
                    <div className="flex flex-wrap gap-1">
                      {trustMetrics.trustSignals.slice(0, 3).map((signal, index) => (
