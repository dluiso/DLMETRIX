...
// AI Content Analysis spinner
export const AIContentSpinner: FC<LoadingSpinnerProps> = ({
  size = 'md',
  className
}) => {
  const sizeClasses = {
  ...
  return (
    <div className={cn('flex items-center justify-center', className)}>
      <div className="relative">
        {/* AI brain-like core */}
        <div className={cn(
          'border-2 border-orange-400 rounded-full flex items-center justify-center bg-gradient-to-br from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20',
          sizeClasses[size]
        )}>
          <svg className="w-4 h-4 text-orange-600 animate-pulse" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M11.49 3.17c-.38-1.56-2.6-1.56-2.98 0a1.532 1.532 0 01-2.286.948c-1.372-.836-2.942.734-2.106 2.106.54.886.061 2.042-.947 2.287-1.561.379-1.561 2.6 0 2.978a1.532 1.532 0 01.947 2.287c-.836 1.372.734 2.942 2.106 2.106a1.532 1.532 0 012.287.947c.379 1.561 2.6 1.561 2.978 0a1.533 1.533 0 012.287-.947c1.372.836 2.942-.734 2.106-2.106a1.533 1.533 0 01.947-2.287c1.561-.379 1.561-2.6 0-2.978a1.532 1.532 0 01-.947-2.287c.836-1.372-.734-2.942-2.106-2.106a1.532 1.532 0 01-2.287-.947zM10 13a3 3 0 100-6 3 3 0 000 6z" clipRule="evenodd" />
          </svg>
        </div>
        
        {/* Orbiting neural network nodes */}
        <div className="absolute inset-0 animate-spin" style={{ animationDuration: '3s' }}>
          <div className="absolute -top-0.5 left-1/2 w-1 h-1 bg-purple-500 rounded-full transform -translate-x-1/2"></div>
          <div className="absolute -right-0.5 top-1/2 w-1 h-1 bg-pink-500 rounded-full transform -translate-y-1/2"></div>
          <div className="absolute -bottom-0.5 left-1/2 w-1 h-1 bg-indigo-500 rounded-full transform -translate-x-1/2"></div>
          <div className="absolute -left-0.5 top-1/2 w-1 h-1 bg-cyan-500 rounded-full transform -translate-y-1/2"></div>
        </div>

        {/* Secondary orbit */}
        <div className="absolute inset-0 animate-spin" style={{ animationDuration: '2s', animationDirection: 'reverse' }}>
          <div className="absolute -top-1 left-1/4 w-0.5 h-0.5 bg-yellow-400 rounded-full"></div>
          <div className="absolute -right-1 top-1/4 w-0.5 h-0.5 bg-red-400 rounded-full"></div>
          <div className="absolute -bottom-1 right-1/4 w-0.5 h-0.5 bg-green-400 rounded-full"></div>
          <div className="absolute -left-1 bottom-1/4 w-0.5 h-0.5 bg-blue-400 rounded-full"></div>
        </div>
      </div>
    </div>
  );
